...
// MARK: - App Blocking Summary Copy
//
// Regression guard for the product decision to stop folding category
// carve-out counts into the summary sentence and the "Currently
// Enforced" stats. The previous UI listed `App exceptions: N` and
// `Web domain exceptions: M` alongside the inputs the user picked to
// block (categories / apps / web domains), which invited a false
// "blocked minus exceptions = unblocked" arithmetic. The summary
// sentence also appended `· N app exceptions` to the base sentence.
//
// The rename-and-reframe pass moved carve-outs into dedicated,
// scoped "Category Exceptions" / "Web Category Exceptions" sections,
// removed the cross-axis counts from `StatusSection`, and stripped the
// exception tail from the summary sentence. These tests lock that
// decision in place: the summary sentence must describe **only**
// inputs the user picked to block. If a future engineer tries to
// re-thread exception counts through this builder, the tests break.
@MainActor
struct AppBlockingSummaryCopyTests {

    @Test("Empty counts produce the no-op sentence")
    func emptyProducesPlaceholder() {
        let sentence = AppBlockingSummaryCopy.summarySentence(
            categoryCount: 0,
            appCount: 0,
            webCount: 0
        )
        #expect(sentence == "No items configured yet.")
    }

    @Test("Single category uses singular noun")
    func singleCategory() {
        let sentence = AppBlockingSummaryCopy.summarySentence(
            categoryCount: 1,
            appCount: 0,
        ...
        #expect(sentence == "Blocking 1 category.")
    }

    @Test("Multiple categories pluralize correctly")
    func multipleCategories() {
        ...
        #expect(sentence == "Blocking 3 categories.")
    }

    @Test("Single app uses singular noun")
    func singleApp() {
        let sentence = AppBlockingSummaryCopy.summarySentence(
            categoryCount: 0,
            appCount: 1,
        ...
        #expect(sentence == "Blocking 1 app.")
    }

    @Test("Multiple apps pluralize correctly")
    func multipleApps() {
        let sentence = AppBlockingSummaryCopy.summarySentence(
            categoryCount: 0,
            appCount: 5,
        ...
        #expect(sentence == "Blocking 5 apps.")
    }

    @Test("Single web domain uses singular noun")
    func singleWebDomain() {
        let sentence = AppBlockingSummaryCopy.summarySentence(
            categoryCount: 0,
            appCount: 0,
            webCount: 1
        )
        #expect(sentence == "Blocking 1 web domain.")
    }

    @Test("Multiple web domains pluralize correctly")
    func multipleWebDomains() {
        let sentence = AppBlockingSummaryCopy.summarySentence(
            categoryCount: 0,
            appCount: 0,
            webCount: 4
        )
        #expect(sentence == "Blocking 4 web domains.")
    }

    @Test("All three axes combine with comma separators")
    func allThreeAxes() {
        let sentence = AppBlockingSummaryCopy.summarySentence(
            categoryCount: 11,
            appCount: 96,
        ...
        #expect(
            sentence == "Blocking 11 categories, 96 apps, 42 web domains."
        )
    }

    @Test("Omits axes with a zero count")
    func omitsZeroAxes() {
        let sentence = AppBlockingSummaryCopy.summarySentence(
            categoryCount: 2,
            appCount: 0,
        ...
        #expect(sentence == "Blocking 2 categories, 3 web domains.")
    }

    /// Explicit negative property: the sentence must never mention
    /// "exception" or "exemptions" — those belong in a separate,
    /// scoped section, not in the summary sentence.
    @Test("Sentence never mentions exceptions")
    func neverMentionsExceptions() {
        let sentence = AppBlockingSummaryCopy.summarySentence(
            categoryCount: 11,
            appCount: 96,
            webCount: 42
        ...
        #expect(
            !sentence.localizedStandardContains("exception")
        )
        #expect(
            !sentence.localizedStandardContains("exempt")
        )
    }
}
